...
The Hawkular OpenShift Agent is designed to allow developers to gather application metrics that they expose for pods running within an OpenShift cluster. This allows for custom metrics to be collected which are not collected as part of the link:https://github.com/openshift/origin-metrics[OpenShift Metrics] installation.

A pod developer just needs to have a Prometheus and/or Jolokia endpoint which exposes metrics, and configure a yaml configfile to specify how these metrics should be collected. 

If a pod is configured with a configmap loaded volume named `hawkular-openshift-agent`, then the agent will automatically read this configmap to determine which metrics to gather and start storing those metrics to the Hawkular Metrics installed as part of the OpenShift Metrics installation.

////
*TODO* provide an updated screencast
Watch link:https://www.youtube.com/watch?v=jvOPlz7lzyM[this quick 10-minute demo] to see the agent in action.
////

[NOTE]
The agent is a complementary component to link:https://github.com/openshift/origin-metrics[Origin Metrics]. Origin Metrics must be installed in OpenShift before the agent can be used.


== Installing

The following sections assume that link:https://github.com/openshift/origin[OpenShift Origin] is already installed with link:https://github.com/openshift/origin-metrics[metrics] enabled.

If you have not yet installed metrics, the link:https://docs.openshift.org/latest/welcome/index.html[OpenShift Origin Documentation] will outline all the link:https://docs.openshift.org/latest/install_config/cluster_metrics.html[steps required].
...
The previous instructions will deploy the agent with the recommended default configuration. For most users this will be the configuration you will want to deploy with.

If you wish to provide more customization to how the agent functions, please see the link:docs/configuration.adoc[advanced configuration] page.
...
Currently there are two example pods which deploy a pod with a Prometheus endpoint and with a Jolokia endpoint. These applications can be found in the link:examples[examples] directory.

These examples can be used as a template to determine how to deploy your own pods with custom metrics.
...
For information on how to build your own agent, please see link:docs/building.adoc[building] documentation.

If you wish to contribute to this project, please see the link:docs/contributing.adoc[contributing] page for information on how to get involved.
